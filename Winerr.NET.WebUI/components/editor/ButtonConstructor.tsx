    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px] bg-zinc-900 border-zinc-800">
                <DialogHeader><DialogTitle>{buttonData ? 'Edit Button' : 'Add New Button'}</DialogTitle></DialogHeader>

                <div className="grid gap-6 py-4">

                    <div className="flex flex-col gap-2">
                        <Label htmlFor="text" className="text-zinc-400">Text</Label>
                        ...
                        <RadioGroup value={type} onValueChange={(v) => setType(v as ButtonConfig['type'])} className="flex flex-wrap gap-x-4 gap-y-2">
                            {supportedButtonTypes && supportedButtonTypes.length > 0 ? supportedButtonTypes.map(btnType => (
                                <div key={btnType} className="flex items-center space-x-2">
                                    ...
                                    <Label htmlFor={`r-${btnType}`} className="font-normal">{btnType}</Label>
                                </div>
                            )) : <p className="text-xs text-zinc-500">No types supported</p>}
                        </RadioGroup>
                    </div>
                    ...
                    <div className="flex flex-row items-center justify-left">
                        <InfoPopover
                            htmlFor="mnemonic"
                            label="Mnemonic"
                            popoverContent="When enabled, draws an underline under the first character of the button's text to indicate it as a hotkey."
                        />
    ...
    return (
        <div ref={setNodeRef} style={style} className="flex items-center justify-between p-2 bg-zinc-800 rounded-md">
            <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs text-zinc-500">{button.type}</span>
